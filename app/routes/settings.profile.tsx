...
import { Alert, Button, FormDescription, FormField, FormLabel, Input, Textarea } from "~/components"
    ...
    <div className="w-full space-y-10">
      <header>
        ...
        <p className="text-muted-foreground">
          Your profiles and links. In Super Duper Gallery, you could have multiple profiles.
        </p>
...
export function UserProfileModeNameForm({
  ...
  const [form, { id, modeName }] = useForm<z.infer<typeof schemaUserProfileModeName>>({
...
export function UserProfileHeadlineForm({
  ...
  const [form, { id, headline }] = useForm<z.infer<typeof schemaUserProfileHeadline>>({
    shouldValidate: "onSubmit",
    lastSubmission: actionData,
    onValidate({ formData }) {
    ...
    <Form {...form.props} replace method="PUT" className="space-y-6">
      <fieldset disabled={isSubmitting} className="space-y-2 disabled:opacity-80">
        ...
        <FormField>
          ...
          />
          <FormDescription>Your headline to recognize your profile</FormDescription>
          {headline.error && (
    ...
    <Form {...form.props} replace method="PUT" className="space-y-6">
      <fieldset disabled={isSubmitting} className="space-y-2 disabled:opacity-80">
        ...
        <FormField>
          ...
          />
          <FormDescription>
            Your bio information about yourself. Can also <span>@mention</span> other users and
            organizations to link to them (later).
          </FormDescription>
          {bio.error && (
...
export async function action({ request }: ActionArgs) {
  ...
  if (intent === "update-user-profile-modename") {
    ...
    const result = await model.userProfile.mutation.updateModeName(submission.value)
  ...
  if (intent === "update-user-profile-headline") {
    ...
    if (!submission.value) return badRequest(submission)
    const result = await model.userProfile.mutation.updateHeadline(submission.value)
    if (result.error) return forbidden({ ...submission, error: result.error })
    return json(submission)
  }
